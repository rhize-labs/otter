...
impl LevelsController {
        ...
        loop {
            ...
            tokio::select! {
                _ = notify_try_compact_chan.recv() => {
                    ...
                    if pick.is_empty() {
                        let _ = zero_level_compact_chan.try_send(());
                    }
                    for p in pick {
    ...
    pub(crate) async fn add_level0_table(&self, table: Table) -> Result<()> {
        ...
        while !self.levels[0].try_add_level0_table(table.clone()).await {
            // Notify compact job
            let _ = notify_try_compact_chan.try_send(());
    ...
    pub(crate) async fn compact_build_tables(
        ...
        defer! {
            let _cost = SystemTime::now().duration_since(execute_time).unwrap().as_millis();
        }
        {
            let cd = cd.read().await;
            ...
            let _is_empty = bot_tables.is_empty();
            ...
            let _tid = random::<u32>();
            let mut count = 0;
            let _cur = tokio::runtime::Handle::current();
            loop {
                // #[cfg(test)]
                // let mut keys = vec![];
        ...
        if first_err.is_err() {
            ...
            let _ = new_tables.iter().for_each(|tb| tb.decr_ref());
            return Err(format!(
    ...
    fn build_change_set(cd: &CompactDef, new_tables: &[Table]) -> Vec<ManifestChange> {
        // new tables to CREATE
        let mut changes = new_tables
            .iter()
            .map(|tb| {
                ManifestChangeBuilder::new(tb.id())
                    .with_level(u32::try_from(cd.next_level.level()).unwrap_or(0))
                    .with_op(Create)
                    .build()
            })
            .collect::<Vec<_>>();
    ...
    fn fill_tables_l0(&self, cd: &mut CompactDef) -> bool {
        ...
        if let Some(slice) = tables.get(left..right) {
            cd.bot.extend(slice.to_vec());
        }
        if cd.bot.is_empty() {
    ...
    fn fill_tables(&self, cd: &mut CompactDef) -> bool {
        ...
        tables.sort_by_key(|b| std::cmp::Reverse(b.size()));
        for t in tables {
    ...
    fn pick_compact_levels(&self) -> Vec<CompactionPriority> {
        ...
        if !self.c_status.overlaps_with(0, &INFO_RANGE) && self.is_level0_compactable() {
            prios.push(CompactionPriority {
                level: 0,
                score: (self.levels.get(0).map_or(0, |l| l.num_tables()) as f64)
                    / (self.opt.num_level_zero_tables as f64),
            });
            info!("level0 will be compacted");
        }
        // stats level 1..n
        for (i, level) in self.levels.get(1..).unwrap_or(&[]).iter().enumerate() {
        ...
        prios.sort_by(|a, b| b.score.partial_cmp(&a.score).unwrap_or(std::cmp::Ordering::Equal));
        prios
    }
    ...
    fn is_level0_compactable(&self) -> bool {
        let compactable = self.levels.get(0).map_or(0, |l| l.num_tables()) >= self.opt.num_level_zero_tables;
        #[cfg(test)]
        debug!(
            "level0 compactable, num_tables: {}, config_tables: {}, yes: {}",
            self.levels.get(0).map_or(0, |l| l.num_tables()),
