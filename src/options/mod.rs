...
pub struct Options {
    ...
    /// 5. Versioning and MVCC options
    /// --------------------------------
    /// Number of versions to keep per key. Setting this to zero means only keep the latest version.
    pub num_versions_to_keep: u32,
    /// Read-only mode. When true, the database will be opened in read-only mode.
    pub read_only: bool,
    /// Encryption key for encrypting/decrypting data. If None, no encryption is used.
