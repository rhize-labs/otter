...
pub(crate) const PTR_ALIGN: usize = 7;

pub trait Allocate: Send + Sync {
    fn alloc(&self, size: usize) -> usize;
    fn alloc_rev(&self, _size: usize) -> usize {
        todo!()
    }
    fn size(&self) -> usize;
...
impl DoubleAlloc {
    pub(crate) fn new(n: usize) -> DoubleAlloc {
        let n = cals_size_with_align(n, PTR_ALIGN);
        assert_eq!(n % (PTR_ALIGN + 1), 0);
        DoubleAlloc {
            head: AtomicUsize::new(PTR_ALIGN + 1),
