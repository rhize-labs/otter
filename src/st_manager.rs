...
impl SkipListManager {
    ...
    pub fn take<'a>(&'a self, p: &'a crossbeam_epoch::Guard) -> Shared<'a, SkipList> {
        ...
        self.mt.as_ref().expect("mt should be initialized").load_consume(p)
    }

    pub fn mt_ref<'a>(&'a self, p: &'a crossbeam_epoch::Guard) -> Shared<'a, SkipList> {
        // self.lock_exclusive();
        // defer! {self.unlock_exclusive()}
        let st = self.mt.as_ref().expect("mt should be initialized").load(Ordering::Relaxed, p);
        st
    }
    ...
    pub fn mt_clone(&self) -> SkipList {
        ...
        unsafe { mt.as_ref().expect("mt should be valid").clone() }
    }
    ...
    pub fn swap_st(&self, opt: Options) {
        ...
        let st = SkipList::new(usize::try_from(opt.arena_size()).unwrap_or(0));
        self.mt
            .as_ref()
            .expect("mt should be initialized")
