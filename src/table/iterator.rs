...
impl BlockIterator {
    ...
    pub fn seek(&self, key: &[u8], whence: IteratorSeek) -> Option<BlockIteratorItem<'_>> {
        match whence {
    ...
    /// Brings us to the first block element that is <= input key.
    pub fn seek_rewind(&self, key: &[u8], whence: IteratorSeek) -> Option<BlockIteratorItem<'_>> {
        match whence {
            IteratorSeek::Origin => self.reset(),
            IteratorSeek::Current => {}
        }
    ...
    fn seek_to_first(&self) -> Option<BlockIteratorItem<'_>> {
        self.reset();
        self.next()
    }

    // todo o shift, too slow, must do something
    fn seek_to_last(&self) -> Option<BlockIteratorItem<'_>> {
    ...
    // TODO Opz Code
    pub(crate) fn next(&self) -> Option<BlockIteratorItem<'_>> {
    ...
    fn prev(&self) -> Option<BlockIteratorItem<'_>> {
        // uninit init
        if self.last_header.borrow().is_none() {
            return None;
        }
