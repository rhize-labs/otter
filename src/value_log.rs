...
impl ValueLogCore {
    ...
    pub(crate) fn incr_iterator_count(&self) {
        let _lock = self._flock.read().unwrap();
    ...
    pub(crate) async fn write(&self, reqs: Vec<Request>) -> Result<()> {
        ...
        for mut req in reqs.into_iter() {
            for (idx, mut entry) in req.entries.into_iter().enumerate() {
                ...
                wt_count += sz;
                ptr.len = u32::try_from(sz).map_err(|_| Error::ValueInvalidRequest)?;
                assert_eq!(
                    buf.get_ref().len() + self.writable_log_offset.load(Ordering::Acquire) as usize,
                    ptr.offset as usize + sz
                );
                if let Some(ptr_slot) = req.ptrs.get(idx) {
                    ptr_slot.store(Some(ptr), Ordering::Release);
                } else {
        ...
        {
            assert!(wt_count == 0 || !self.buf.read().await.get_ref().is_empty());
            ...
            // update log
            self.writable_log_offset
                .fetch_add(u32::try_from(n).map_err(|_| Error::ValueInvalidRequest)?, Ordering::Release);
            ...
            if self.writable_log_offset.load(Ordering::Acquire)
                > u32::try_from(self.opt.value_log_file_size).map_err(|_| Error::ValueInvalidRequest)?
            {
    ...
    async fn rewrite(&self, lf: TArcRW<LogFile>, _x: &KVCore) -> Result<()> {
        ...
        loop {
            ...
            let mut entry = entries.get_mut(0).ok_or(Error::ValueInvalidRequest)?.0.clone();
            ...
            vptr.dec(&mut Cursor::new(&vs.value))?;
    ...
    async fn pick_log(&self) -> Option<TArcRW<LogFile>> {
        ...
        let vlog = vlogs_guard.vlogs.get(fid).ok_or(Error::ValueInvalidRequest).ok()?;
        Some(vlog.clone())
    }

    pub(crate) async fn pick_log_by_vlog_id(&self, id: &u32) -> Result<TArcRW<LogFile>> {
        let pick_vlogs = self.pick_log_guard().await;
        let vlogs = pick_vlogs.vlogs.get(id);
        let vlog = vlogs.ok_or(Error::ValueInvalidRequest)?;
        Ok(vlog.clone())
    }

    // Note: it not including dirty file
    async fn pick_log_guard(&self) -> PickVlogsGuardsReadLock<'_> {
        let vlogs = self.vlogs.read().await;
        let vlogs_fids = vlogs.keys().copied().collect::<HashSet<_>>();
    ...
    fn parse_file_ids(data_files: &mut [String]) -> Vec<u64> {
    ...
    pub(crate) async fn wait_on_gc(&self, lc: Closer) {
        ...
        let _ = self.garbage_ch.send(()).await;
    }

    /// only one gc worker
    pub async fn trigger_gc(&self, gc_threshold: f64) -> Result<()> {
        match self.garbage_ch.try_send(()) {
            Ok(()) => {
                let ok = self.do_run_gc(gc_threshold).await;
                let _ = self.garbage_ch.recv().await;
                ok
            }
    ...
    pub async fn do_run_gc(&self, gc_threshold: f64) -> Result<()> {
        ...
        let skip_first_m = f64::from(thread_rng_n(u32::try_from(self.opt.value_log_file_size / M).map_err(|_| Error::ValueInvalidRequest)?)) - window;
        let mut skipped = 0.0;
        ...
        let mut fid = 0;
        {
            ...
            while let Ok((entry, vptr)) = fut.recv().await {
                let esz = f64::from(vptr.len) / f64::from(1 << 20); // in MBs, +4 for the CAS stuff.
                skipped += esz;
                ...
                if start.elapsed().map_or(false, |d| d.as_secs() > 10) {
                    wg.signal_and_wait().await;
                    return Ok(());
                }
                // Get the late value
                ...
                let mut vptr = vptr; // TODO avoid copy
