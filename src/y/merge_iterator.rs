...
impl MergeIterator {
    ...
    fn init_heap_by_indexes(&self, stack: Vec<usize>) {
        for index in stack {
            if let Some(item) = self.itrs.get(index).and_then(|itr| itr.peek()) {
                self.push_item_into_heap(index, item);
            }
        }
    }
    ...
    fn push_item_into_heap(&self, index: usize, item: IteratorItem) {
        if self.heap_flag.borrow().get(index).map_or(false, |&flag| flag) {
            return;
        }

        if let Some(flag) = self.heap_flag.borrow_mut().get_mut(index) {
            *flag = true;
        }
    ...
    fn pop_item_from_heap(&self) -> Option<IterRef> {
        if let Some(item) = self.heap.borrow_mut().pop() {
            if let Some(flag) = self.heap_flag.borrow_mut().get_mut(item.index) {
                *flag = false;
            }
            return Some(item);
        }
